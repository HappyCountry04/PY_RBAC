...
export default function RoleManagementPage() {
  ...
  async function handleStatusChange(row: Record<string, unknown>) {
    ...
    if (!await modalConfirm(`确认要"${text}""${row.roleName}"角色吗？`)) return;
  ...
  async function handleDelete(row?: Record<string, unknown>) {
    ...
    if (!await modalConfirm(`是否确认删除角色编号为"${ids.join(",")}"的数据项？`)) return;
    ...
    <SidebarLayout currentPaths={["role"]}>
      <section className="data-surface">
        ...
        <div className="table-wrap"><table><thead><tr>
          ...
          {loading ? <TableSkeleton cols={8} rows={6} /> : rows.length ? rows.map((row) => { const rid = Number(row.roleId); const isAdmin = rid === 1;
...
import { flattenTreeOptions, statusOptions, buildTree } from "../../shared/utils";
import TreeCheck from "../../shared/components/tree-check";
import type { TreeNode } from "../../shared/components/tree-check";

function toTreeCheckNodes(data: Record<string, unknown>[]): { id: string | number; label: string; children?: { id: string | number; label: string; children?: { id: string | number; label: string }[] }[] }[] {
  return data.map((n) => ({
    id: n.id ?? n.menuId as number,
    label: String(n.label ?? n.menuName ?? ""),
    children: (n.children as Record<string, unknown>[])?.length ? toTreeCheckNodes(n.children as Record<string, unknown>[]) : undefined,
  }));
}

function getAllMenuIds(nodes: Record<string, unknown>[]): number[] {
  const ids: number[] = [];
  (function walk(items: Record<string, unknown>[]) {
    for (const n of items) {
      const id = n.id ?? n.menuId;
      if (id !== undefined) ids.push(Number(id));
      if (n.children) walk(n.children as Record<string, unknown>[]);
    }
  })(nodes);
  return ids;
}
...
function RoleEditModal({ mode, roleId, onClose, onSaved }: { mode: "create" | "edit"; roleId?: number; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useS<Record<string, string>>({ roleSort: "0", status: "0" });
  const [error, setError] = useS("");
  const [busy, setBusy] = useS(false);
  const [loading, setLoading] = useS(mode === "edit");
  const [menuTree, setMenuTree] = useS<Record<string, unknown>[]>([]);
  const [menuChecked, setMenuChecked] = useS<Set<number>>(new Set());
  const [menuStrictly, setMenuStrictly] = useS(true);

  useEf(() => {
    (async () => {
      try {
        if (mode === "edit" && roleId) {
          const [treeRes, detail] = await Promise.all([
            api.get(`/system/menu/roleMenuTreeselect/${roleId}`),
            api.get(`/system/role/${roleId}`),
          ]);
          const tr = treeRes as any;
          setMenuTree((tr.menus ?? []));
          if (tr.checkedKeys) setMenuChecked(new Set((tr.checkedKeys as number[]).map(Number)));
          const d = detail as any;
          const data = (d.data ?? d) as any;
          setValues({
            roleName: String(data.roleName ?? ""),
            roleKey: String(data.roleKey ?? ""),
            roleSort: String(data.roleSort ?? 0),
            status: String(data.status ?? "0"),
            dataScope: String(data.dataScope ?? "1"),
            menuCheckStrictly: data.menuCheckStrictly ? "true" : "false",
            remark: String(data.remark ?? ""),
          });
          setMenuStrictly(data.menuCheckStrictly !== false);
          if (d.menuIds) setMenuChecked(new Set((d.menuIds as number[]).map(Number)));
        } else {
          const menus = await api.get("/system/menu/treeselect");
          setMenuTree(((menus as any).data ?? []) as Record<string, unknown>[]);
        }
      } catch (err) {
  ...
  const allMenuIds = useMemo(() => getAllMenuIds(menuTree), [menuTree]);
  const treeCheckNodes = useMemo(() => toTreeCheckNodes(menuTree), [menuTree]);

  function handleToggleCheck(id: number, checked: boolean) {
    const next = new Set(menuChecked);
    if (checked) next.add(id); else next.delete(id);
    setMenuChecked(next);
  }

  function handleCheckAll(ids: number[], checked: boolean) {
    const next = new Set(menuChecked);
    for (const id of ids) {
      if (checked) next.add(id); else next.delete(id);
    }
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!values.roleName?.trim()) { setError("角色名称不能为空"); return; }
    if (!values.roleKey?.trim()) { setError("权限字符不能为空"); return; }
    setBusy(true); setError("");
    const body: any = { ...values, menuIds: [...menuChecked], roleSort: Number(values.roleSort), menuCheckStrictly: menuStrictly };
    try {
      if (mode === "edit" && roleId) { body.roleId = roleId; await api.put("/system/role", body); showToast("修改成功", "success"); }
      else { await api.post("/system/role", body); showToast("新增成功", "success"); }
      onSaved();
    } catch (err) {
  ...
  if (loading) return (
    <div className="modal-mask">
      <div className="modal-panel" style={{ display: "grid", placeItems: "center", minHeight: 200 }}>
        <p>加载中...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-mask">
      <form className="modal-panel" style={{ width: "min(680px, 100%)" }} onSubmit={submit}>
        <div className="modal-head">
          <h2>{mode === "edit" ? "修改角色" : "添加角色"}</h2>
          <button type="button" className="text-button" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="form-grid">
          <label><span className="form-label-text">角色名称 <em className="required">*</em></span><input value={values.roleName ?? ""} onChange={(e) => setValues((p) => ({ ...p, roleName: e.target.value }))} /></label>
          <label><span className="form-label-text">权限字符 <em className="required">*</em></span><input value={values.roleKey ?? ""} onChange={(e) => setValues((p) => ({ ...p, roleKey: e.target.value }))} /></label>
          <label><span className="form-label-text">角色顺序 <em className="required">*</em></span><input type="number" value={values.roleSort ?? "0"} onChange={(e) => setValues((p) => ({ ...p, roleSort: e.target.value }))} /></label>
          <label><span className="form-label-text">状态</span>
            <div className="radio-group">
              {statusOptions.map((o) => (
                <label key={o.value} className="radio-label">
                  <input type="radio" name="status" value={o.value} checked={(values.status || "0") === o.value} onChange={(e) => setValues((p) => ({ ...p, status: e.target.value }))} />
                  {o.label}
                </label>
              ))}
            </div>
          </label>
          <label className="row-full"><span className="form-label-text">菜单权限</span>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 4, alignItems: "center" }}>
                <label className="check-item" style={{ margin: 0 }}>
                  <input type="checkbox" checked={menuStrictly} onChange={(e) => setMenuStrictly(e.target.checked)} />
                  <span>父子联动</span>
                </label>
                <span style={{ color: "var(--muted)", fontSize: 12 }}>取消联动可独立选择父子节点</span>
              </div>
              <TreeCheck nodes={treeCheckNodes} checked={menuChecked} onCheck={handleToggleCheck} onCheckAll={handleCheckAll} />
            </div>
          </label>
        </div>
...
function toDeptTreeNodes(data: Record<string, unknown>[]): TreeNode[] {
  return data.map((n) => ({
    id: n.id ?? n.deptId as number,
    label: String(n.label ?? n.deptName ?? ""),
    children: (n.children as Record<string, unknown>[])?.length ? toDeptTreeNodes(n.children as Record<string, unknown>[]) : undefined,
  }));
}

function RoleDataScopeModal({ role, onClose, onSaved }: { role: Record<string, unknown>; onClose: () => void; onSaved: () => void }) {
  const [dataScope, setDataScope] = useS(String(role.dataScope ?? "1"));
  const [deptTree, setDeptTree] = useS<Record<string, unknown>[]>([]);
  const [deptChecked, setDeptChecked] = useS<Set<number>>(new Set());
  ...
  useEf(() => {
    (async () => {
      try {
        const res = await api.get(`/system/role/deptTree/${role.roleId}`) as any;
        setDeptTree((res.depts ?? res.data ?? []) as Record<string, unknown>[]);
        if (res.checkedKeys) setDeptChecked(new Set((res.checkedKeys as number[]).map(Number)));
      } catch (err) {
        setError(err instanceof ApiError ? err.message : "加载失败");
      } finally {
        setLoading(false);
  ...
  const deptNodes = useMemo(() => toDeptTreeNodes(deptTree), [deptTree]);
  const allDeptIds = useMemo(() => getAllMenuIds(deptTree), [deptTree]);

  function handleDeptCheck(id: number, checked: boolean) {
    const next = new Set(deptChecked);
    if (checked) next.add(id); else next.delete(id);
    setDeptChecked(next);
  }
  function handleDeptCheckAll(ids: number[], checked: boolean) {
    const next = new Set(deptChecked);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true); setError("");
    try {
      const deptIds = dataScope === "2" ? [...deptChecked] : [];
      await api.put("/system/role/dataScope", { roleId: role.roleId, dataScope, deptIds });
  ...
  if (loading) return (
    <div className="modal-mask">
      <div className="modal-panel" style={{ display: "grid", placeItems: "center", minHeight: 200 }}><p>加载中...</p></div>
    </div>
  );
  ...
  return (
    <div className="modal-mask">
      <form className="modal-panel" style={{ width: "min(560px, 100%)" }} onSubmit={submit}>
        <div className="modal-head">
          <h2>分配数据权限</h2>
          <button type="button" className="text-button" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="form-grid">
          <label>角色名称<input value={String(role.roleName ?? "")} disabled /></label>
          <label>权限字符<input value={String(role.roleKey ?? "")} disabled /></label>
